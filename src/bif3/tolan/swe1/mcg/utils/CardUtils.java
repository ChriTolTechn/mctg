...
public class CardUtils {
    ...
    public static Card buildCard(String cardId, String name, float damage) throws InvalidCardParameterException {
        List<String> nameSplit = Arrays.asList(name.split(CommonRegex.SPLIT_STRING_BY_UPPERCASE_LETTERS));
        ...
        try {
            if (nameSplit.size() > 0) {
                if (nameSplit.size() == 2) {
                    element = ElementType.valueOf(nameSplit.get(0));
                    nameSplit.remove(0);
                }

                type = CardType.valueOf(nameSplit.get(0));
            }
